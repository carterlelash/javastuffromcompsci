...
class starter {
	...
	public static void searchArrInt(ArrayList <Integer> x, int y) {
		System.out.println("");
		for (int u = 0; u < x.size(); u++) {
			if (x.get(u) == y)  {
				System.out.println(y + " is present in this array at the index: " + u);
			}
		}
	}
	
	public static void searchArrString(ArrayList <String> x, String y) {
		for (int u = 0; u < x.size(); u++) {
			if (x.get(u).equals(y)) {
				System.out.println(y + " is present in this array at the index: " + u);
			}
